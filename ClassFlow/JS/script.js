...
form.addEventListener('submit', async (e) => {
    ...
    try {
        const aluno = await fetch(`http://10.106.208.30:3000/alunos?nome=${encodeURIComponent(nome)}`);
        const dados_alunos = await aluno.json();
        const docente = await fetch(`http://10.106.208.30:3000/docentes?nome=${encodeURIComponent(nome)}`);
